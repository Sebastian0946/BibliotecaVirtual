...
exports.validarCamposLibros = void 0;
const validarCamposLibros = (req, res, next) => {
    ...
    if (req.method === 'POST') {
        ...
        if (!ISBN)
            camposFaltantes.push('ISBN');
        if (!Autor)
            camposFaltantes.push('Autor');
        if (!Copias)
            camposFaltantes.push('Copias');
        if (!Editorial)
            camposFaltantes.push('Editorial');
        if (!FechaPublicacion)
            camposFaltantes.push('FechaPublicacion');
        if (!Titulo)
...
exports.validarCamposLibros = validarCamposLibros;
